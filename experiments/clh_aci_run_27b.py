"""T-094 runner: CLH end-to-end on ACI-BENCH (outpatient) against a local vLLM (Qwen3.6-27B).
Added by reproduction work; not part of upstream codeseeker. Mirrors clh_run_27b.py
(MDACE) with the ACI-BENCH dataset + FY2025 catalog (ACI gold is ICD-10-CM FY2024+).
This is the project backbone -> the CLH outpatient baseline that improvement methods must beat.
...
- BYTE-LEVEL DETOK REPAIR: vllm-q36 leaks GPT-2 byte-BPE markers in free generation;
  reverse them client-side so the model can THINK freely and agents parse clean text.
Usage: python clh_aci_run_27b.py          # smoke debug=True (10 test notes)
       python clh_aci_run_27b.py --full   # full ACI-BENCH test split (120 notes)
"""
import os
import sys
...
args = benchmark.Arguments(
    ...
    experiment_name=("clh-qwen27b-aci-test-full" if FULL else "clh-qwen27b-aci-smoke"),
