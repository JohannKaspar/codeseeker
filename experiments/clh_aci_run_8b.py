"""T-094 runner: CLH end-to-end on ACI-BENCH (outpatient) against a local vLLM (CLH-small 8B).
Added by reproduction work; not part of upstream codeseeker. Mirrors clh_run_8b.py
...
Usage: python clh_aci_run_8b.py          # smoke debug=True (10 test notes)
       python clh_aci_run_8b.py --full   # full ACI-BENCH test split (120 notes)
"""
import os
import sys
...
VLLM = {
    ...
    "deployment": "deepseek-ai/DeepSeek-R1-0528-Qwen3-8B",
    "use_cache": False,
}
args = benchmark.Arguments(
    experiment_id="clh-repro",
    experiment_name=("clh-small-8b-aci-test-full" if FULL else "clh-small-8b-aci-smoke"),
